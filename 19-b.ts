...
reader.on('close', () => { 
  const lines : {first : number, last : number}[] = []
  const size = 100
  ...
  while (true) {
    ...
    if (last - first > size && lines[y - size + 1].last - first + 1 >= size) {
      console.log((lines[y - size + 1].last - size + 1) * 10000 + (y - size + 1)) 
      break
    }
    y++
    x = first
