...
enum type { walked, teleported }
interface point { x : number, y : number }
interface location { point : point, orientation: orientation }
interface path { id : string, steps : number, how : type}
...
const shortest_distance = (portals: Map<string, portal>, start: portal, finish: portal) => {
  const to_search = [{ portal: start, steps : 0, path : [hash_portal(start.id)], level : 0 } ]
  const visited : Map<string, number> = new Map()

  while (to_search.length > 0) {
    const current = to_search.shift()
    if (current == undefined) throw 'WHAT!'
    if (current.portal == finish && current.level == 0) return current.steps

    const previous = visited.get(hash_portal(current.portal.id) + '|' + current.level)
    if (previous != undefined && previous < current.steps) continue
    visited.set(hash_portal(current.portal.id) + '|' + current.level, current.steps)

    current.portal.paths.forEach(path => {
      const next = portals.get(path.id)
      if (next == undefined) throw 'WHAT!'
      const next_path = [...current.path]
      next_path.push(hash_portal(next.id))
      const next_level = current.level + ((path.how == type.teleported)?(next.id.number==1?-1:1):0)
      if (next_level >= 0) {
        to_search.push({portal : next, steps : current.steps + path.steps, path : next_path, level : next_level })
      }
    })
  }
}
...
const create_paths = (map : string[], portals: Map<string, portal>) => {
  [...portals.keys()].forEach(name => {
    ...
    if (other != undefined)
      portal.paths.push({ id : hash_portal(other.id), steps : 1, how : type.teleported })
  })
}
  ...
  while (to_search.length > 0) {
    ...
    if (map[current.point.y][current.point.x] == '.') {
      ...
      if (other != undefined && hash_portal(other.id) != hash_portal(portal.id)) {
        portal.paths.push({id : hash_portal(other.id), steps : current.steps, how : type.walked})
      }
    }
  }
}
  ...
  for (let y = 0; y < map.length; y++) {
    for (let x = 0; x < map[y].length; x++) {
      if (x < map[y].length - 1 && map[y][x].match(/[A-Z]/) && map[y][x + 1].match(/[A-Z]/)) {
        ...
        const number = (x == 0 || y == 0 || x == map[y].length - 2 || y == map.length - 2) ? 0 : 1
        const portal = {id : {name, number}, location : {point : {x, y}, orientation : orientation.horizontal}, paths : []}
        
        if (portals.get(hash_portal(portal.id)) == undefined)
        ...
        const number = (x == 0 || y == 0 || x == map[y].length - 2 || y == map.length - 2) ? 0 : 1
        const portal = {id : {name, number}, location : {point : {x, y}, orientation : orientation.vertical}, paths : []}
