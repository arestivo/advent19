...
reader.on('close', () => { 
  ...
  const display = new screen(true, true)

  go(computer, { x : 0, y : 0}, display)

  const oxi = [...maze].filter(m => m[1] == 2)[0][0]
  const pos = {x : parseInt(oxi.split(',')[0]), y : parseInt(oxi.split(',')[1])}

  search(pos, 0, display)

  console.log(Math.max(...[...distances].map(d => d[1])))
})

const search = (pos : point, distance : number, display : screen) => {
  if (maze.get(hash(pos)) == 0) return

  const last_distance = distances.get(hash(pos))
  if (last_distance != undefined && last_distance < distance) return

  distances.set(hash(pos), distance)

  display.output(pos.x, pos.y, 4)

  search({ x : pos.x + 1, y : pos.y }, distance + 1, display)
  search({ x : pos.x - 1, y : pos.y }, distance + 1, display)
  search({ x : pos.x, y : pos.y + 1 }, distance + 1, display)
  search({ x : pos.x, y : pos.y - 1}, distance + 1, display)
}
